...
from p2_tasks import task2, task3, phase1
...
#   - We can use either phase-2 task-2(i.e type-weight semantics) or 
#     task-3(i.e type-type similarity semantics)
...
def run(args):
    ...
    for y in y_x_count:
        ...
        if l > max_y:
            max_y = l
            args.y = y
    print("Using subject {} images for computing latent semantics".format(args.y))
    print("Learnt TYPES: {}".format(y_x_count[args.y]))
    # Get Latent space representing path1 images
    path1_output = os.path.join(args.path1, "task-1-output")
    os.makedirs(path1_output, exist_ok=True)
    latent_file = os.path.join(
        path1_output, "{}-{}-subject-{}-top-{}-type-semantics.pkl".format(args.dim_reduce, model, args.y, args.k))
    data = {}
    ...
    path2_output = os.path.join(args.path2, "task-1-output")
    ...
    for image_name, fd in path2_images_fd.items():
        xyz = image_name.split(".")[0].split("-")
        x, y, z = xyz[1], xyz[2], xyz[3]
        expected_testing_labels.append(x)
        if args.dim_reduce is not None:
            transformed_fd = transform_fd(np.asarray(fd).ravel(), top_k_latent_semantics)
        else:
            transformed_fd = np.asarray(fd).ravel()
    ...
    if args.classifier == "svm":
        ...
        for i in range(len(rankMatrix)):
            ...
            labels = uniqueLabels
    compute_results(args, labels, expected_testing_labels, training_labels, ypred, path2_images_fd, path2_output, model)
