...
def task3(args, print_flag=True):
    ...
    neigh = NearestNeighbors(n_neighbors=k, radius=1000.0, algorithm='ball_tree',
                             leaf_size=400, metric=metric, p=2, metric_params=None, n_jobs=-1)
    try:
    ...
    if print_flag:
        print("{:<25}| {:<20}| {:<20}|".format("IMAGE", "OVERALL-SCORE", "DISTANCE-SCORE"))
        print("------------------------------------------------------")
    score = k
    table_data = []
    for x, y in zip(neigh_ind[0], neigh_dist[0]):
        ...
        table_data.append([index_to_image[x], score, y])
        image_list.append(index_to_image[x])
        score -= 1
    if print_flag:
        for row in table_data:
            print("{:<25}| {:<20.3f}| {:<20.3f}|".format(*row))
