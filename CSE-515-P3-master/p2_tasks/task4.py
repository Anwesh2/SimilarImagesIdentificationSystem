...
def run(args):
    ...
    file = os.path.join(args.image_dir, "phase-2/task-4-output")
    os.makedirs(file, exist_ok=True)
    similarity_on_whole_data_nd = {}
    task_4_out = os.path.join(
        args.image_dir, "phase-2/task-4-output", "nd_{}.pkl".format(model))
    filename = None
    ...
    # for s1 in feed_fd_2:
    #     sim_score1 = []
    #     for s1_img in feed_fd_2[s1]:
    #         sim_score2 = []
    #         for s2 in feed_fd_2:
    #             dist = 0
    #             for s2_img in feed_fd_2[s2]:
    #                 dist = dist + euclidean(s1_img, s2_img)
    #             sim_score2.append(dist/len(feed_fd_2[s2]))
    #         if sim_score1:
    #             sim_score1 = list(np.asarray(sim_score1) +
    #                               np.asarray(sim_score2))
    #         else:
    #             sim_score1 = sim_score2
    #     sim_score1 = list(np.asarray(sim_score1)/len(feed_fd_2[s1]))
    #     subject_subject.append(sim_score1)
